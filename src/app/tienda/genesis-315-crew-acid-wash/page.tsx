...
const getImagePath = (diseno: string, view: 'front' | 'back') => {
  ...
  if (d === 'color') return `/catalogo/genesis315_crew_acidwash/black_baw_${view}.png`; // fallback to baw since color wasn't provided
  if (d === 'blanco y negro' || d === 'baw') return `/catalogo/genesis315_crew_acidwash/black_baw_${view}.png`;

  return `/catalogo/genesis315_crew_acidwash/black_baw_${view}.png`;
};
...
export default function ProductPage() {
  ...
  const handleAddToCart = () => {
    ...
    addToCart({
      id: `g315-crew-${selectedBase}-${selectedDiseno}-${selectedTalla}`.toLowerCase().replace(/ /g, '-'),
      name: "Genesis 3:15 - Crew Acid Wash",
      price: 20000,
    ...
    <div ref={containerRef} className="pt-32 pb-24 bg-white min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="product-anim flex items-center gap-2 text-sm text-charcoal/50 font-mono mb-8">
          ...
          <span>/</span>
          <span className="text-charcoal font-semibold">Genesis 3:15 - Crew Acid Wash</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="product-anim sticky top-32">
            <div className="relative aspect-[4/5] bg-neutral-100 rounded-3xl overflow-hidden border border-black/5 shadow-sm">
              <Image
                ...
                alt={`Genesis 3:15 Crew Acid Wash vista ${selectedView}`}
                fill
          ...
          <div className="product-anim flex flex-col pt-4">
            <h1 className="text-4xl md:text-5xl font-sans font-bold text-charcoal mb-4">
              Genesis 3:15 <span className="font-serif italic text-black font-medium block mt-2">Crew Acid Wash</span>
            </h1>
            <p className="text-2xl font-mono font-semibold text-charcoal mb-6">$20.000 <span className="text-sm text-charcoal/40 font-normal">CLP</span></p>

            <p className="text-charcoal/70 text-lg mb-10 leading-relaxed">
              Crewneck de estética 'Acid Wash' altamente detallado e inspirado en la gran promesa redentora. Construcción excepcional de alto gramaje para soportar las condiciones más exigentes, abrigando con firmeza. Impresión DTF resistente y rica en texturas.
            </p>
              ...
              >
                {isAdding ? (
                  ...
                  <>
                    ...
                    Añadir al Carrito — ${(20000 * quantity).toLocaleString('es-CL')}
                  </>
                )}
              </button>
            </div>

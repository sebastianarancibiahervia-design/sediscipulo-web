...
import { Link, ShoppingCart, Check, ShieldCheck, Undo2, Truck, Plus, Minus } from "lucide-react";
import { useCart } from "@/components/CartProvider";
import gsap from "gsap";

const TALLAS = ["S", "M", "L", "XL"];
const COLORES_BASE = ["Chocolate", "Arena", "Negro"];

const COLORES_DISENO: Record<string, string[]> = {
  Chocolate: ["Beige", "Blanco"],
  Arena: ["Café", "Blanco", "Negro"],
  Negro: ["Gris"],
};

const getImagePath = (base: string, diseno: string, view: 'front' | 'back') => {
  const b = base.toLowerCase();
  const d = diseno.toLowerCase();

  // Specific mappings depending on what user provided
  if (b === 'arena' && (d === 'café' || d === 'cafe')) return `/catalogo/maranata_back_oversized/arena_cafe_${view}.png`;
  if (b === 'chocolate' && d === 'beige') return `/catalogo/maranata_back_oversized/chocolate_beige_${view}.png`;
  if (b === 'negro' && d === 'gris') return `/catalogo/maranata_back_oversized/negro_gris_${view}.png`;

  // Fallbacks avoiding crashes for undefined combinations
  if (b === 'arena') return `/catalogo/maranata_back_oversized/arena_cafe_${view}.png`;
  if (b === 'chocolate') return `/catalogo/maranata_back_oversized/chocolate_beige_${view}.png`;
  if (b === 'negro') return `/catalogo/maranata_back_oversized/negro_gris_${view}.png`;

  return `/catalogo/maranata_back_oversized/negro_gris_${view}.png`;
};

export default function ProductPage() {
  const { addToCart } = useCart();
  const [selectedTalla, setSelectedTalla] = useState<string>("L");
  const [selectedBase, setSelectedBase] = useState<string>("Chocolate");
  const [selectedDiseno, setSelectedDiseno] = useState<string>("Beige");
  ...
  const handleAddToCart = () => {
    ...
    addToCart({
      id: `maranata-back-${selectedBase}-${selectedTalla}`.toLowerCase(),
      name: "Maranata Back Oversized",
      price: 18000,
      image: getImagePath(selectedBase, selectedDiseno, selectedView),
      base: selectedBase,
    ...
    <div ref={containerRef} className="pt-32 pb-24 bg-white min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="product-anim flex items-center gap-2 text-sm text-charcoal/50 font-mono mb-8">
          ...
          <span>/</span>
          <span className="text-charcoal font-semibold">Maranata Back Oversized</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="product-anim sticky top-32">
            <div className="relative aspect-[4/5] bg-neutral-100 rounded-3xl overflow-hidden border border-black/5 shadow-sm">
              {/* Product Image */}
              <Image
                src={getImagePath(selectedBase, selectedDiseno, selectedView)}
                alt={`Maranata Back Oversized en color ${selectedBase} vista ${selectedView}`}
                fill
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div onClick={() => setSelectedView('front')} className={`aspect-[4/5] rounded-xl bg-neutral-100 border ${selectedView === 'front' ? 'border-charcoal' : 'border-black/5 hover:border-black/20'} overflow-hidden relative cursor-pointer transition-all`}>
                <Image
                  src={getImagePath(selectedBase, selectedDiseno, 'front')}
                  alt="Thumbnail Front"
                  fill
              ...
              <div onClick={() => setSelectedView('back')} className={`aspect-[4/5] rounded-xl bg-neutral-100 border ${selectedView === 'back' ? 'border-charcoal' : 'border-black/5 hover:border-black/20'} overflow-hidden relative cursor-pointer transition-all`}>
                <Image
                  src={getImagePath(selectedBase, selectedDiseno, 'back')}
                  alt="Thumbnail Back"
                  fill
          ...
          <div className="product-anim flex flex-col pt-4">
            <h1 className="text-4xl md:text-5xl font-sans font-bold text-charcoal mb-4">
              Maranata Back <span className="font-serif italic text-black font-medium">Oversized</span>
            </h1>
            <p className="text-2xl font-mono font-semibold text-charcoal mb-6">$18.000 <span className="text-sm text-charcoal/40 font-normal">CLP</span></p>

            <p className="text-charcoal/70 text-lg mb-10 leading-relaxed">
              Polera de algodón premium con diseño exclusivo de inspiración bíblica "Maranata". Corte oversized para máxima comodidad, ideal para uso diario o eventos congregacionales. Acabado en DTF de alta calidad que asegura durabilidad gráfica tras múltiples lavados.
            </p>
              ...
              >
                {isAdding ? (
                  ...
                  <>
                    ...
                    Añadir al Carrito — ${(18000 * quantity).toLocaleString('es-CL')}
                  </>
                )}
              </button>
            </div>
